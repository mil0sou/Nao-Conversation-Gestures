...
motion.wakeUp() #makes the robot track your face
...
def checkface(val): #checking if the user is facing or not
    if val and isinstance(val, list) and len(val) >= 2:
        faceInfoArray = val[1] #getting face points
        for faceInfo in faceInfoArray:
            if isinstance(faceInfo, list) and len(faceInfo) >= 2:
                ...
                if leftEyePoints != rightEyePoints:
                    diffdg1 = round(abs((leftEyePoints[0] - rightEyePoints[0])),2) #substracting
                    ...
                    if facing1 or facing2:
                        ...
                        if total_sum < DISTANCE_TRESHOLD:
                            ...
                            facing = False # facing but too far away
                        #tts.say("Hi!")
                    else:
                       #print("visage detecte mais pas de face")
                       facing = False # detected but not facing
                else: 
                    #print("visage semi visible ")
                    facing = False # half visible 
   
    else:
        #print("Aucun visage detecte")
        facing = False # no face detected
    return facing
...
while True:
    ...
    client_socket.sendall(facingstr.encode()) #sending facing information





"""def checknear(val): # not used anymore
    if val and isinstance(val, list) and len(val) >= 2:
        faceInfoArray = val[1]
        for faceInfo in faceInfoArray:
            if isinstance(faceInfo, list) and len(faceInfo) >= 2:
